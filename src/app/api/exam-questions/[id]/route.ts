...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return NextResponse.json(await prisma.examQuestion.update({ where: { id: parseInt(id) }, data: d }))
}

export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  await prisma.examQuestion.delete({ where: { id: parseInt((await params).id) } })
